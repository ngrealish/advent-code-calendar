...
export class Day2Component implements OnInit {
  ...
  puzzle1() {
    let i = 0;
    this.arr.forEach(p => {
      const min = p.split(': ')[0].split(' ')[0].split('-')[0];
      const max = p.split(': ')[0].split(' ')[0].split('-')[1];
  ...
  puzzle2() {
    let i = 0;
    this.arr.forEach(p => {
      const pos1 = parseInt(p.split(': ')[0].split(' ')[0].split('-')[0], 0);
      const pos2 = parseInt(p.split(': ')[0].split(' ')[0].split('-')[1], 0);
      const letter = p.split(': ')[0].split(' ')[1];
      const password = p.split(': ')[1];
      const passwordArr = password.split('');
      const match = [];
      passwordArr.forEach((l, j) => {
        const letPos = j + 1;
        if (pos1 === letPos && letter === l) {
          match.push(true);
        }
        if (pos2 === letPos && letter === l) {
          match.push(true);
        }
      });
      if (match.filter(m => (m)).length === 1) {
        i++;
      }
    });
    console.log('Valid Passwords', i);
