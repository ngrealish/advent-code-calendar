...
export class Day6Component implements OnInit {
  ...
  puzzle1() {
    ...
    let totalAnswers = 0;
    this.answers.forEach(group => {
      const groupAnswers = [];
      console.log(group);
      group.forEach(person => {
        person.split('').forEach(a => {
          if (groupAnswers.indexOf(a) <= -1) {
            groupAnswers.push(a);
          }
        });
      });
      console.log(groupAnswers.length);
      totalAnswers += groupAnswers.length;
    });
    this.p1num = totalAnswers;
  ...
  puzzle2() {
    ...
    this.answers.forEach(group => {
      ...
      group.forEach(person => {
        person.split('').forEach(a => {
          if (this.isIsAllOtherGroupAnswers(group, a)) {
            if (unique.indexOf(a) < 0) {
              unique.push(a);
            }
          }
        });
      });
      totalUniqueAnswers += unique.length;
    });

    this.p2num = totalUniqueAnswers;
    console.log(this.p2num);
  }

  isIsAllOtherGroupAnswers(group, a) {
